...
def jaccard_1(y_true, y_pred):
    #y_pred_pos = K.round(y_pred)#clip剪切
    y_pred_pos=y_pred
    intersection = K.sum(y_true[:,:,3] * y_pred_pos[:,:,3], axis=[0, -1, -2])
    sum_ = K.sum(y_true[:,:,3] + y_pred_pos[:,:,3], axis=[0, -1, -2]) 
    jac = (intersection + smooth) / (sum_ - intersection + smooth)
    return K.mean(jac)
...
def calc_jacc(model,xval,yval):
    ...
    prd = model.predict(xval, batch_size=4)
    print('shape: ',prd.shape, yval.shape)
    sum_=0
    for i in range(N_Cls):
        t_msk = yval[:, :, :,i]
        t_prd = prd[:, :, :,i]
        t_msk = t_msk.reshape(yval.shape[0] * yval.shape[1], yval.shape[2])
        t_prd = t_prd.reshape(prd.shape[0] * prd.shape[1], prd.shape[2])
        jk = jaccard_npcoef(t_msk, t_prd)
        sum_+=jk
    return sum_/6.0
'''
...
if __name__ == '__main__':
    ...
    '''
    msk=model.predict(x_val,batch_size=4)
    start=time.time()
    prd = model.predict(img, batch_size=4)
    end=time.time()
    tt=end-start
